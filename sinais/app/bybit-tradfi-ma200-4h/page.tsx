...
export default function BybitTradfiMa2004hPage() {
  const [items, setItems] = useState<TradfiMa2004hItem[]>([]);
  ...
  const fetchFromDb = async () => {
    try {
      const response = await fetch('/api/bybit-tradfi-ma200-4h');
  ...
  const handleRefresh = async () => {
    try {
      setRefreshing(true);
      setError('');
      const response = await fetch('/api/bybit-tradfi-ma200-4h', { method: 'POST' });
      const data = await response.json();
      if (response.ok && data.success) {
        setItems(data.items || []);
