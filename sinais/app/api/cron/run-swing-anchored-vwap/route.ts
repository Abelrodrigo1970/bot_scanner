...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await runSwingAnchoredVwap15mPipeline({
      logPrefix: '[cron swing-vwap]',
    });
    ...
    console.error('Erro swing-vwap:', error);
    return NextResponse.json(
      {
        error: 'Ocorreu um erro ao executar Swing Anchored VWAP 15m',
