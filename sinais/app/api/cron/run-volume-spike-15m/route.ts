 ...
 * Universo: tabela `BybitAboveMa200Mc20m` (menu Bybit Volume >20M e MA200 1h).
 */
async function runMaCross5mInBackground(
  ...
  try {
    ...
    if (maRows.length === 0) {
      console.warn(
        '[MA Cross 15m BG] Nenhum símbolo no scan Bybit Volume >20M e MA200 1h. Atualize essa página antes.'
      );
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: 'MA Cross 15m iniciado em background (universo: scan Bybit Volume >20M e MA200 1h)',
