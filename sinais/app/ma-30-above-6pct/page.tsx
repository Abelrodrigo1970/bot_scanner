...
export default function Ma30Above6PctPage() {
  const [items, setItems] = useState<Ma30Above6Item[]>([]);
  ...
  const fetchFromDb = async () => {
    try {
      const response = await fetch('/api/ma-30-above-6pct');
  ...
  const handleRefresh = async () => {
    try {
      setRefreshing(true);
      setError('');
      const response = await fetch('/api/ma-30-above-6pct', { method: 'POST' });
      const data = await response.json();
      if (response.ok && data.success) {
        setItems(data.items || []);
