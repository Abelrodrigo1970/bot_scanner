...
export default function BybitMa2004hVolumePage() {
  const [items, setItems] = useState<BybitMa2004hItem[]>([]);
  ...
  const fetchFromDb = async () => {
    try {
      const response = await fetch('/api/bybit-ma200-4h-volume');
  ...
  const handleRefresh = async () => {
    try {
      setRefreshing(true);
      setError('');
      const response = await fetch('/api/bybit-ma200-4h-volume', { method: 'POST' });
      const data = await response.json();
      if (response.ok && data.success) {
        setItems(data.items || []);
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
              Bybit Volume 4h (2M) + MA200 (4h)
            </h1>
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Criptos Bybit com turnover da última 4h acima de 2M USDT e preço acima da MA200 (4h)
            </p>
          </div>
          <button
        ...
        <div className="mb-6 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-4">
          ...
          <ul className="text-xs text-blue-700 dark:text-blue-400 space-y-1 list-disc list-inside">
            ...
            <li>Liquidez: turnover da última vela 4h da Bybit, mínimo 2M USDT</li>
            <li>Técnico: preço de fecho acima da MA200 no timeframe 4h (vela fechada)</li>
            <li>Ordenação: turnover 4h descendente</li>
          </ul>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            <p className="text-gray-600 dark:text-gray-400">
              Nenhum dado. Clica em &quot;Atualizar Scan&quot; para executar o filtro 4h.
            </p>
          </div>
        ) : (
              ...
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-50 dark:bg-gray-900">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Turnover 4h</th>
                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">Preço</th>
                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">MA200 4h</th>
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {items.map((item) => (
                    <tr key={item.symbol} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm text-gray-900 dark:text-white">${formatTurnover(item.turnover4h)}</td>
