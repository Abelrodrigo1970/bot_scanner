/**
 * Estudo: engolfo NEW vs OLD — Scanner 2
 *
 * NEW (pedido): bear + drop≥1% + fecho < EMA12 e < EMA21 + dist preço vs MA21 < 10%
 * OLD (live):   bear + drop≥1% + EMA12 < EMA21 + fecho < EMA21
 *
 * SAI: SL +10% | TP1 −20%@50% | resto 24h | fee 0,1%
 *
 * Uso: node scripts/study-engolfo-below-mas-dist10-scanner2.mjs
 */
...
const MIN_DROP = 1;
/** Distância preço–MA21 tem de ser inferior a 10% */
...
async function main() {
  ...
  console.log(
    `ENGOLFO NEW (fecho <12&21, dist21<${MAX_DIST_BELOW_21}%) vs OLD × Scanner 2 | ${new Date(startMs).toISOString().slice(0, 10)} → ${new Date(endMs).toISOString().slice(0, 10)}`
  );
  console.log(
    `Candidatos ${CANDIDATE_LIMIT} vol≥${(MIN_QUOTE_VOL / 1e3).toFixed(0)}k | S2 top ${SCANNER2_TOP} / 4h`
  );
  console.log('═'.repeat(88));
  ...
  for (const [sym, c15] of series) {
    ...
    for (let i = 100; i < c15.length - 1; i++) {
      ...
      if (belowBoth && notTooDeep && i - lastNew >= COOLDOWN_BARS) {
  ...
  const sNew = summarize(
    `NEW × Scanner 2: fecho < MA12 & MA21 + dist21 < ${MAX_DIST_BELOW_21}%`,
    tradesNew
  );
  const sOld = summarize('OLD × Scanner 2: EMA12<EMA21 + fecho<EMA21', tradesOld);

  const best = [...tradesNew].sort((a, b) => b.pnl - a.pnl).slice(0, 8);
  const worst = [...tradesNew].sort((a, b) => a.pnl - b.pnl).slice(0, 8);
  console.log('\nTop wins NEW×S2:');
  for (const t of best)
    console.log(
      `  ${t.day} ${t.sym} ${t.pnl >= 0 ? '+' : ''}${t.pnl.toFixed(2)}% ${t.path} drop=${t.drop}% dist21=${t.dist21}%`
    );
  console.log('Top losses NEW×S2:');
  ...
  const buckets = [
    ...
    { name: '2–4%', lo: 2, hi: 4 },
    { name: '4–7%', lo: 4, hi: 7 },
    { name: '7–10%', lo: 7, hi: 10 },
  ];
  const byDist = [];
  console.log('\nNEW×S2 por dist abaixo MA21:');
  for (const b of buckets) {
    const sub = tradesNew.filter((t) => t.dist21 >= b.lo && t.dist21 < b.hi);
  ...
  const withStack = tradesNew.filter((t) => t.stack12lt21);
  const withoutStack = tradesNew.filter((t) => !t.stack12lt21);
  const stackSum = (arr) =>
    arr.length
      ? {
          n: arr.length,
  ...
  const out = {
    ...
    rule: 'bear + drop≥1% + close < EMA12 & EMA21 + dist(price,EMA21) < 10%',
    candidates: symbols.length,
    seriesOk: series.size,
    snapshots4h: snaps.size,
    new: sNew,
    old: sOld,
    newWithStack12lt21: stackSum(withStack),
    newWithoutStack12lt21: stackSum(withoutStack),
    byDist,
    best: best.map((t) => ({
      day: t.day,
      sym: t.sym,
  ...
  const path = new URL('./out-engolfo-below-mas-dist10-scanner2.json', import.meta.url);
