/**
 * rsi_vendido LONG 15m — bounce — últimos 60 dias
 * Universo: Scanner 6 (fecho > SMA80 4h), top 40 por |% vs MA|
 * Entrada: após RSI fechar < 28, comprar na 1ª vela em que o RSI sobe
 *   ex. 27 → 22 (não) → 27 (compra)
 * SL −5% | TP1 +10% (30%) | TP2 +48% (30%)
 * Resto: RSI cruza para baixo da SMA(14) do RSI com RSI > 65
 *
 * Uso: node scripts/study-rsi-vendido-15m-bounce-60d.mjs
 */
...
const OUT = path.join(
  ...
  'out-rsi-vendido-15m-bounce-60d.json'
);
const CACHE = path.join(
  path.dirname(fileURLToPath(import.meta.url)),
  'cache-rsi-vendido-15m-60d-klines.json'
);
...
async function main() {
  ...
  console.log(
    `rsi_vendido 15m BOUNCE · Scanner 6 top ${TOP_N} · ${new Date(t0).toISOString().slice(0, 10)} → ${new Date(t1).toISOString().slice(0, 10)}`
  );
  console.log(
    `1ª subida RSI após <${ENTRY} | SL −${SL}% | TP1 +${TP1}% ${TP1_POS}% | TP2 +${TP2}% ${TP2_POS}% | resto RSI×SMA${RSI_MA_P} down RSI>${TRAIL_MIN} | fee ${FEE}% | size $${SIZE}`
  );
  console.log('═'.repeat(96));
  ...
  try {
    ...
    if (raw?.meta?.days === DAYS && Array.isArray(raw.symbols) && raw.h4 && raw.m15) {
      symbols = raw.symbols;
      for (const [sym, bars] of Object.entries(raw.h4)) h4BySym.set(sym, bars);
      for (const [sym, bars] of Object.entries(raw.m15)) m15BySym.set(sym, bars);
      loadedCache = true;
      console.log(`Cache klines: ${m15BySym.size} símbolos (${CACHE})`);
    }
  } catch {
    /* no cache */
  }
  ...
  if (!loadedCache) {
    symbols = await topLinearSymbols();
    ...
    const cachePayload = {
      meta: { days: DAYS, savedAt: new Date().toISOString() },
      symbols,
      h4: Object.fromEntries(h4BySym),
      m15: Object.fromEntries(m15BySym),
    };
    writeFileSync(CACHE, JSON.stringify(cachePayload));
    console.log(`\nCache gravado: ${CACHE}`);
  }
  console.log(`Símbolos com dados: ${m15BySym.size}`);
  ...
  for (const [sym, m15] of m15BySym) {
    ...
    let pendingFirstUp = false; // 1ª vela com RSI a subir neste episódio < 28
    ...
    for (let i = RSI_P + RSI_MA_P + 2; i < closed.length; i++) {
      ...
      if (curr < ENTRY) {
        ...
        inOversold = false;
        pendingFirstUp = false;
      }

      const firstUp = pendingFirstUp && curr > prev;
      if (firstUp) {
        pendingFirstUp = false; // consome a 1ª vela positiva do episódio
        if (bar.t >= t0 && bar.t <= t1 && !isBusy && inUniverse(sym, bar.t)) {
  ...
  const payload = {
    meta: {
      ...
      rules: {
        ...
        entry: `1ª subida RSI após fecho < ${ENTRY} (ex. 27→22 não, 22→27 sim)`,
        sl: SL,
