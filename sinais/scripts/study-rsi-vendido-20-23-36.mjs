/**
 * Mergulho: entra RSI 4h cruza abaixo de 23, 22 ou 20;
 * sai cruza acima de 32 ou 36. SL −5%, hold 24h.
 *
 * Uso: node scripts/study-rsi-vendido-20-23-36.mjs
 */
...
function summarize(label, trades) {
  if (!trades.length) {
    console.log(`\n${label}: 0 trades`);
    return { label, n: 0, avg: 0, usdt: 0, wr: 0, nTrim: 0, avgTrim: 0, usdtTrim: 0, byPath: {} };
  }
...
async function main() {
  ...
  console.log(
    `Mergulho <23/<22/<20 → sair >32/>36 | ${new Date(t0).toISOString().slice(0, 10)} → ${new Date(t1).toISOString().slice(0, 10)}`
  );
  console.log('═'.repeat(88));
  ...
  fs.writeFileSync(
    'scripts/out-rsi-vendido-20-23-36.json',
    JSON.stringify({ summaries, buckets }, null, 2)
  );
  console.log('\nJSON: scripts/out-rsi-vendido-20-23-36.json');
}
