/**
 * Estudo: TREND_PULLBACK no universo Scanner 6 — Binance USDT-M Futures
 *
 * Universo (Scanner 6 / UNIVERSE_ABOVE_MA80_4H):
 *  - candidatos: top volume USDT-M, vol 24h ≥ 500k
 *  - filtro dinâmico na entrada: fecho 4h fechado > SMA80 (4h)
 *
 * Setup (sinais/lib/scanner.ts):
 ...
 *
 * Uso: node scripts/study-trend-pullback-scanner6-binance.mjs
 */

const BINANCE = 'https://fapi.binance.com';
const FEE = 0.1;
const SIZE = 100;
const CANDIDATE_LIMIT = 200; // Scanner 6 usa 400; 200 p/ velocidade do estudo
const MIN_QUOTE_VOL = 500_000; // Scanner 6
...
/** Última vela 4h já fechada em t15 (Scanner 6 usa fecho fechado). */
...
async function main() {
  ...
  console.log(
    `TREND_PULLBACK × Scanner 6 (SMA80 4h) | ${new Date(startMs).toISOString().slice(0, 10)} → ${new Date(endMs).toISOString().slice(0, 10)}`
  );
  console.log(
    `Candidatos top ${CANDIDATE_LIMIT} vol≥${(MIN_QUOTE_VOL / 1e3).toFixed(0)}k | gate close>SMA80 4h | fee ${FEE}% | $100 | minScore ${MIN_SCORE}`
  );
  console.log('═'.repeat(88));
  ...
  for (let si = 0; si < symbols.length; si++) {
    ...
    try {
      ...
      c4h = await fetchKlines(sym, '4h', startMs - warmMs, endMs);
    } catch {
      continue;
    }
    if (c1h.length < 220 || c15.length < 100 || c4h.length < SMA80_PERIOD + 5) continue;
    scanned++;
    ...
    const sma80_4h = smaSeries(closes4h, SMA80_PERIOD);
    ...
    for (let i = 50; i < c15.length; i++) {
      ...
      // Scanner 6 gate: última 4h fechada acima SMA80
      const i4 = findClosed4hIndex(c4h, t);
      if (i4 < SMA80_PERIOD - 1 || sma80_4h[i4] == null) continue;
      if (c4h[i4].c <= sma80_4h[i4]) continue;
      inUniverseBars++;
      ...
      const pctFromMa = ((c4h[i4].c - sma80_4h[i4]) / sma80_4h[i4]) * 100;
      ...
      const base = {
        ...
        day,
        atrPct,
        pctFromMa80_4h: +pctFromMa.toFixed(2),
      };
  ...
  console.log(
    `Scanned OK: ${scanned} | universe-bar checks: ${inUniverseBars} | entries: ${entries}`
  );
  ...
  for (const t of best)
    console.log(
      `  ${t.day} ${t.sym} ${t.side} ${t.pnl >= 0 ? '+' : ''}${t.pnl.toFixed(2)}% ${t.path} score=${t.score} pctMA=${t.pctFromMa80_4h}%`
    );
  ...
  const out = {
    ...
    universe: 'Scanner6_ABOVE_SMA80_4H',
  ...
  const path = new URL('./out-trend-pullback-scanner6-binance.json', import.meta.url);
