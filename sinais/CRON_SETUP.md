...
| `/api/cron/run-1h` | Pivot Boss Bear 15m (Scanner 1, SL 7%) | `0 * * * *` (24h) |

| `/api/cron/run-universe-scans` | Scanner 1 (universo) | `0 */4 * * *` (24h) |



**Desactivados** (remover do cron-job.org se existirem): `run-30m`, `run-afastamento-30m`, `run-rsi-15m`, `run-scans-ma`.
